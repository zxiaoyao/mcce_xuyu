...
/* normal pw is guaranteed to be smaller than 2000. When it is bigger than 5000, it is
 * represents a value referenced to a clashed pair of conformers.
 */
...
typedef struct {
    ...
    float *vdw;
    float *ele;
} MFE;  // all the mfe energy terms of a residue
...
typedef struct {
	char *protonationStates;
	int counter;
} PState;
PState old_pstate;        // the protonation states of residues.

int   load_ms_gold(STRINGS *str, const char *ms_fname);
int   update_conf_id(unsigned short *conf_id, int *state);
int   write_ms(MSRECORD *ms_state);
int writeBits(char *charArray, int arrayLen, FILE *out_fp);
void MC_smp(int n);

STRINGS ms_spe_lst;        // global vairable, saving names of marked residues to save microstates.
FILE   *ms_fp;
FILE   *bit_fp;
FILE   *pro_fp;      // protonation states, protonation.txt
/* for the microstate */
...
int monte()
{
    ...
    if (env.ms_out) {
    	...
    	ms_spe_lst.n = 0;
    	if (load_ms_gold(&ms_spe_lst, FN_MS_GOLD)) {
...
//    		}
    		...
    		bit_fp = fopen(FN_BIT_OUT, "wb");
    		fwrite(&ms_spe_lst.n, 1, sizeof(int), bit_fp);
    		for (i_spe=0; i_spe<ms_spe_lst.n; i_spe++) {
    			fwrite(ms_spe_lst.strings[i_spe], 8, sizeof(char), bit_fp);
    		}

//    		pro_fp = fopen(FN_PROTONATION, "w");
//    		for (i_spe=0; i_spe<ms_spe_lst.n; i_spe++) {
//    			fprintf(pro_fp, "%10s", ms_spe_lst.strings[i_spe]);
//    		}
//    		fprintf(pro_fp, "%10s\n", "count");
    		/* change "NSTATE_MAX" to -1 to force the program to run MC sampling, instead of analytic solution
    		 * so that the microstates are saved.
    		 */
    		env.nstate_max = -1;
    	}
    }
...
int load_conflist()
{
    ...
    /** conf.iConf must be consistent with the index in energy.opp of the conformer,
     * otherwise there would be a mismatch of conformers. e.g. when two lines in head3.lst are
     * exchanged after running step3.
     */
    while(fgets(sbuff, sizeof(sbuff), fp)) {
        /* load this line to a conf template */
        if (strlen(sbuff) < 20) continue;
        sscanf(sbuff, "%5d %14s %c %4f%7f%6f%6f%3d%3d%8f%8f%8f%8f%8f%8f %s", &conf_temp.iConf,
        conf_temp.uniqID,
        &conf_temp.on,
        &conf_temp.occ,
        &conf_temp.netcrg,
...
{  int i, j;
   ...
   // load the pairwise interaction again, round the ele and vdw to keep consistent with mfe.py  
    ...
    fprintf(fp, "       pKa/Em  n(slope) 1000*chi2      vdw0    vdw1    tors    ebkb    dsol   offset  pHpK0   EhEm0    -TS   residues  vdw     ele       total\n");
...
int print_mfe(int i_res, float mfeP, FILE *pK_fp, FILE *res_fp)
{
    ...
    if (abs(i_high-i_low) == 0) { // do mfe at only one point
        get_mfe(i_res, i_low); 
        if (env.titr_type == 'p') 
            fprintf(pK_fp, "  %8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%10.2f\n", 
                   ...
                   mfe_res[i_res].mfe.vdw_sum/PH2KCAL, 
                   mfe_res[i_res].mfe.ele_sum/PH2KCAL, 
                   mfe_res[i_res].mfe.total/PH2KCAL);
        else 
            fprintf(pK_fp, "  %8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%8.1f%10.1f\n", 
                   ...
                   mfe_res[i_res].mfe.vdw_sum/mev2Kcal, 
                   mfe_res[i_res].mfe.ele_sum/mev2Kcal, 
        ...
        old_mfe.mfe.vdw_sum  = mfe_res[i_res].mfe.vdw_sum;
        old_mfe.mfe.ele_sum  = mfe_res[i_res].mfe.ele_sum;
        ...
        mfe_res[i_res].mfe.vdw_sum  = old_mfe.mfe.vdw_sum*(1-ratio) + mfe_res[i_res].mfe.vdw_sum*ratio;
        mfe_res[i_res].mfe.ele_sum  = old_mfe.mfe.ele_sum*(1-ratio) + mfe_res[i_res].mfe.ele_sum*ratio;
...
int get_mfe(int i_res, int t_point)
{
    ...
    mfe_vdw = (float *) calloc(mfe_res[i_res].n, sizeof(float));
    mfe_ele = (float *) calloc(mfe_res[i_res].n, sizeof(float));
    ...
    for (j=0; j<mfe_res[i_res].n; j++) {
        ...
        for (k=0; k<conflist.n_conf; k++) {
                ...
                mfe_vdw[j] += pairwise_vdw[mfe_res[i_res].conf[j]][conflist.conf[k].iConf] * fround3(occ_table[k][t_point]);
                mfe_ele[j] += pairwise_ele[mfe_res[i_res].conf[j]][conflist.conf[k].iConf] * fround3(occ_table[k][t_point]);
        }
    ...
    for (j=0; j<mfe_res[i_res].n; j++) {
        if (strchr(conflist.conf[mfe_res[i_res].conf[j]].uniqID, '+') || strchr(conflist.conf[mfe_res[i_res].conf[j]].uniqID, '-')) {
            ...
            E_ionize.vdw_sum  += nocc[j] * mfe_vdw[j];           
            E_ionize.ele_sum  += nocc[j] * mfe_ele[j];           
            ...
            for (k=0; k<n_all; k++) {
                for (q=0; q<all_res[k].n; q++) {
                	if (!strcmp(conflist.conf[mfe_res[i_res].conf[0]].uniqID, "Ub101M0311_001")) {
                		if (!strcmp(conflist.conf[all_res[k].conf[q]].uniqID, "ALA01M0221_001")) {
                			printf("%s, %s,", conflist.conf[mfe_res[i_res].conf[0]].uniqID, conflist.conf[all_res[k].conf[q]].uniqID);
                			printf("%.2f\n", pairwise[mfe_res[i_res].conf[0]][all_res[k].conf[q]]);
                		}
                	}
                	if (!strcmp(conflist.conf[mfe_res[i_res].conf[1]].uniqID, "Ub1-1M0311_002")) {
                  	    if (!strcmp(conflist.conf[all_res[k].conf[q]].uniqID, "ALA01M0221_001")) {
                	        printf("%s, %s,", conflist.conf[mfe_res[i_res].conf[1]].uniqID, conflist.conf[all_res[k].conf[q]].uniqID);
                            printf("%.3f, %.3f, %.3f\n", pairwise_vdw[mfe_res[i_res].conf[j]][all_res[k].conf[q]],
                            							 pairwise_ele[mfe_res[i_res].conf[j]][all_res[k].conf[q]],
                            							 pairwise[mfe_res[i_res].conf[1]][all_res[k].conf[q]]);
                	    }
                	}
            ...
            E_ground.vdw_sum  += nocc[j] * mfe_vdw[j];           
            E_ground.ele_sum  += nocc[j] * mfe_ele[j];           
    ...
    mfe_res[i_res].mfe.vdw_sum  = E_ionize.vdw_sum - E_ground.vdw_sum;
    mfe_res[i_res].mfe.ele_sum  = E_ionize.ele_sum - E_ground.ele_sum;
...
int group_conftype()
{
	...
	// If conformers of the same kind are not staying together in head3.lst, this would be a problem.
	/* real grouping */
	oldID = get_conftype(conflist.conf[0]);
	n = 0;
...
int load_ms_gold(STRINGS *str, const char *ms_fname)
{
    ...
    if ((ms_gold_fp=fopen(ms_fname, "r"))) {
...
//    printf("   cycles= %d, ites= %d\n", cycles, n_cycle);
    for (i=0; i<cycles; i++) {
        ...
        i*n_cycle, E_minimum+E_base, E_state+E_base, get_E()+E_base);
        */
        fprintf(fp, "Step %10d, E_minimum = %10.2f, E_running = %10.2f, E_base = %10.2f\n",
        i*n_cycle, E_minimum + E_base, E_state + E_base, E_base);
        fflush(fp);
        iters = n_cycle;
        ...
        while (iters) {
            ...
            if (dE < 0.0 || (float) rand()/RAND_MAX < exp(b*dE)) {   /* go to new low */
            	// write the previous state first
                if (ms_state.counter != 0) {
                	write_ms(&ms_state);
                }

                // update the conf ids from the new state
...
int write_ms(MSRECORD *ms_state)
{
    ...
    char *protonationStates = (char *) calloc(ms_spe_lst.n, sizeof(char));

    for (i_spe=0; i_spe<ms_spe_lst.n; i_spe++) {
        //fwrite(&ms_state->conf_id[i_spe], 1, sizeof(unsigned short), ms_fp);
        if (conflist.conf[ms_state->conf_id[i_spe]].H == 0)
        	protonationStates[i_spe] = 0;
        else
        	protonationStates[i_spe] = 1;
        //fprintf(pro_fp, "%15s", conflist.conf[ms_state->conf_id[i_spe]].uniqID);
    }
    //fprintf(pro_fp, "%10d\n", ms_state->counter);

    writeBits(protonationStates, ms_spe_lst.n, bit_fp);
    fwrite(&ms_state->counter, 1, sizeof(int), bit_fp);
...
//    printf("count %d\n", ms_state->counter);
    //free(protonationStates);

    return 0;
}

void readbinary(char byte)
{
	int i=0;
	for (i=0;i<8;i++) {
...
{
	/**
	 * Convert an array of 0s and 1s into bytes,
	 * and then write the bytes in a binary file.
	 */
	int byteNumber = arrayLen / 8;
	int leftBits = arrayLen % 8;

	int iByte = 0, iBit = 0;
	for (iByte=0; iByte<byteNumber; iByte++) {
		char eightBits = 0;
		for (iBit=0; iBit<8; iBit++) {
			if (charArray[iByte*8 + iBit] != 0) {
				eightBits |= (1 << (7 - iBit));
				//printf("FFFFFFFFFFFFFFFF, %d\n", iBit);
				//readbinary(eightBits);
				//exit(2);
			}
		}

		fwrite(&eightBits, 1, sizeof(char), out_fp);
	}

	if (leftBits != 0) {
		// Add one more byte if there are still bits left.
		char eightBits = 0;
		for (iBit=0; iBit<leftBits; iBit++) {
			if (charArray[byteNumber*8 + iBit] != 0) {
				eightBits |= (1 << (7 - iBit));
			}
		}
		fwrite(&eightBits, 1, sizeof(char), out_fp);
