...
int rotamers()
{
	...
	if (env.rotamer_seed > 0)
		idum = env.rotamer_seed;
	else
		idum = time(NULL);
	...
	if (mkdir(ROT_DIR, 0755)) {
		printf(" Can't make directory rot\n");
	}

	/* make a directory for conformer making statistics */
   ...
   return 0;
	...
	make_rotamer_statistics(&confstat, &prot);

	write_pdb_headlist("rmdup", prot);
   ...
   step when there are multiple conformers in step1_out - Yifan 10/18/2007 */
	...
	make_rotamer_statistics(&confstat, &prot);
	write_pdb_headlist("rotate", prot);
	...
	for (kr=0; kr<prot.n_res; kr++) {
		while(place_missing_res(prot,kr,1) > 0);
...
//	}
	...
	write_pdb_headlist("repack", prot);
...
for (i=0; i<prot.n_res; i++) {
	...
	// rotations = 0 or 1, xzhu
	if (prot.res[i].rotations < 2) continue;
		...
		for (j_res=0; j_res<i_res; j_res++) {
			...
			for (j_ngh=0; j_ngh<prot.res[j_res].n_ngh; j_ngh++) {
				if (i_res == prot.res[j_res].ngh[j_ngh]->i_res_prot) {
					...
					prot.res[i_res].ngh = (RES **) realloc(prot.res[i_res].ngh, n_ngh*sizeof(void *));
...
for (kr=0; kr<prot.n_res; kr++) {
	for (kc=1; kc<prot.res[kr].n_conf; kc++) {
		...
		// don't check netcrg of conformer, one can check it in head2.lst
		/*
...
int over_vdw(PROT prot, int i_res, int i_conf, int j_conf, float cutoff)
{
	...
	for (i_ngh = 0; i_ngh < prot.res[i_res].n_ngh; i_ngh++) {
		...
		for (k_conf=0; k_conf < prot.res[k_res].n_conf; k_conf++) {
			...
			Ei = Evdw_conf2conf(prot, i_res, i_conf, k_res, k_conf) * env.s2_vdw;
			Ej = Evdw_conf2conf(prot, i_res, j_conf, k_res, k_conf) * env.s2_vdw;
...
int write_pdb_headlist(char *fn, PROT prot)
{
	...
	sprintf(pdb_name, "%s/%s.pdb", ROT_DIR, fn);
	sprintf(hd_name,  "%s/%s.lst", ROT_DIR, fn);
...
/**
 * Wrtie rotamer making statistics to the file ROTSTAT.
 * @param confstat variable stores rotamer making statistics.
 * @param prot protein data structure.
 */
void make_rotamer_statistics(CONFSTAT *confstat, PROT *prot)
{
	FILE *fp = fopen(ROTSTAT,"w");
	if (fp) {
		fprintf(fp,"   Rotamer making statistics:\n");
		write_step2stat(fp, *prot, *confstat);
		printf("\n");
		fclose(fp);
	}
