...
int monte()
{
	...
	MC_occ = (float **) calloc(env.monte_runs, sizeof(float *));
	for (i=0; i<env.monte_runs; i++) {
		if (!(MC_occ[i] = (float *) calloc(conflist.n_conf, sizeof(float)))) {
	...
	sigma = (float *) calloc(conflist.n_conf, sizeof(float *));
	...
	for (i=0; i<env.titr_steps; i++) {
		...
		for (j=0; j<conflist.n_conf; j++) {
			...
			fprintf(fp, "%s   %c %8.3f%10.3f", conflist.conf[j].uniqID,
					conflist.conf[j].on,
					conflist.conf[j].E_self0, conflist.conf[j].E_self);
...
float get_base()
{
	...
	/*for (kc=0; kc<conflist.n_conf; kc++) {
    	printf("get_base, %s, %.3f, %.3f, %.3f, %.3f\n", conflist.conf[kc].uniqID, conflist.conf[kc].E_TS,
    			conflist.conf[kc].E_mfe, conflist.conf[kc].E_self0, conflist.conf[kc].E_self);
    }*/
...
void MC(int n)
{
	...
	//    int ib;
	//    for (i=0; i<n_free; i++) {
	//    	printf("%s: %d, %d\n", conflist.conf[free_res[i].conf[0]].uniqID, biglist[i].n, free_res[i].n);
	//    	for (ib=0; ib<biglist[i].n; ib++) {
	//    		printf("%s\t", conflist.conf[free_res[biglist[i].res[ib]].conf[0]].uniqID);
	//    	}
	//    	printf("\n");
	//    }
	/* clear counters */
	...
	for (i=0; i<cycles; i++) {
		...
		while (iters) {
            ...
            int ic;
            for (ic=0; ic<n_free; ic++) {
            	printf("%s, E_self: %.3f\n", conflist.conf[state[ic]].uniqID, conflist.conf[state[ic]].E_self);
            }
			 ...
			 */
			//            if (0) {   /* do multiple flip if odd number */
	...
	fprintf(fp, "Exit %10d, E_minimum = %10.3f, E_base = %10.3f, E_running = %10.3f\n", n_total, E_minimum+E_base, E_base, E_state+E_base);
	...
	free(old_state);
	//    printf("final random: %d, MC time: %ld\n", rand(), time(NULL) - xzA);
	return;
}
...
void MC_smp(int n)
{
	...
	fprintf(fp, "Exit %10d, E_minimum = %10.2f, E_base = %10.3f, E_running = %10.2f\n", n_total, E_minimum+E_base, E_base, E_state+E_base);
	fprintf(fp, "The average running energy, corresponding to H, is %8.3f kCal/mol\n", H_average+E_base);
	fflush(fp);
